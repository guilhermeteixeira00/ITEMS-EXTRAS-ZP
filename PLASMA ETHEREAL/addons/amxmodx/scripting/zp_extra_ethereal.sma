...
new const Fire_snd[] = {"weapons/ethereal/ethereal-1.wav"}
new const went[] ={"weapon_ump45"}
new const GUNSHOT_DECALS[] = { 41, 42, 43, 44, 45 }

new ethereal_V_MODEL[64] = "models/ethereal/v_ethereal.mdl"
new ethereal_P_MODEL[64] = "models/ethereal/p_ethereal.mdl"
new ethereal_W_MODEL[64] = "models/ethereal/w_ethereal.mdl"

new cvar_recoil_ethereal, cvar_dmg_ethereal, g_itemid_ethereal, cvar_clip_ethereal, cvar_ethereal_ammo, cvar_oneround
...
public plugin_init()
{
	...
	/* -> Take Damage com ReAPI -> */
	//RegisterHookChain(RG_CBasePlayer_TakeDamage, "RG_CBasePlayer_TakeDamage_Pre", true);

	/* -> Take Damage sem ReAPI -> */
	...
	cvar_dmg_ethereal = register_cvar("zp_Ethereal_dmg", "50")
	cvar_clip_ethereal = register_cvar("zp_Ethereal_clip", "25")
	cvar_ethereal_ammo = register_cvar("zp_Ethereal_ammo", "100")
	cvar_oneround = register_cvar("zp_Ethereal_one_round", "1")
	g_itemid_ethereal = zp_register_extra_item("Ethereal", 120, ZP_TEAM_HUMAN)
...
public plugin_precache()
{
	...
	precache_sound("weapons/ethereal/ethereal_draw.wav")
	precache_sound("weapons/ethereal/ethereal_idle1.wav")
	precache_sound("weapons/ethereal/ethereal_reload.wav")
	m_iBlood[0] = precache_model("sprites/ethereal/blood.spr")
	//m_iBlood[1] = precache_model("sprites/bloodspray.spr")
	register_forward(FM_PrecacheEvent, "fwPrecacheEvent_Post", 1)
	g_beamSpr = precache_model("sprites/zbeam4.spr")
	precache_generic("sprites/ethereal/weapon_ethereal.txt")
	precache_generic("sprites/ethereal/640hud2.spr")
	precache_generic("sprites/ethereal/640hud74.spr")
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type) {
	...
	if(!zp_get_user_zombie(victim) || get_user_weapon(attacker) != wId)
		return HAM_IGNORED

	damage *= get_pcvar_float(cvar_dmg_ethereal)
...
/* ~ [ Com REAPI ] ~ */
/*public RG_CBasePlayer_TakeDamage_Pre(const iVictim, iInflictor, iAttacker, Float: flDamage, bitsDamageType)
{
	// Não irá usar esses parametros
	#pragma unused iInflictor, bitsDamageType

	// Se a vitima estiver viva e o cara com arma não conseguir causar dano, retonar
	if(is_user_alive(iVictim) && !rg_is_player_can_takedamage(iAttacker, iVictim))
		return HC_CONTINUE;

	// Se o cara com a arma estiver com a arma (wId = CSW_UMP45), e tiver comprado o extra item
	if(get_user_weapon(iAttacker) == wId && g_has_ethereal[iAttacker])
	{
		// Retornar o valor flutuante (parametro Float: flDamage (dano causado pela arma) vezes o dano da cvar)
		new Float: flNewDamage = (flDamage * get_pcvar_float(cvar_dmg_ethereal));
		SetHookChainReturn(ATYPE_FLOAT, flNewDamage);
	}
